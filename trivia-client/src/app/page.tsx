...
import { useGameStore, GameState } from "@/store/gameStore"; // Import GameState type
...
export default function JoinPage() {
  ...
  const initializeLobby = useGameStore(
    (state: GameState) => state.initializeLobby
  );
  ...
  const joinLobby = async (gameCode: string, nickname: string) => {
    const response = await fetch("http://localhost:8000/lobby/join", {
      ...
      body: JSON.stringify({ code: gameCode, nickname }),
    });
    if (!response.ok) {
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        const lobbyData = await joinLobby(trimmedCode, trimmedNickname);
        initializeLobby(
          trimmedNickname,
          trimmedCode,
          lobbyData.players,
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 text-white p-8">
      <div className="text-center mb-12">
        ...
        <p className="text-xl text-purple-300 font-light">Join the Showdown!</p>
      </div>
      ...
      >
        ...
        <div className="mb-6">
          ...
          <input
            ...
            className={`w-full px-4 py-3 bg-gray-700 border rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:border-transparent transition duration-200 ${
              errorMessage && !nickname.trim()
                ? "border-red-500 ring-red-500"
                : "border-gray-600 focus:ring-purple-500" // Highlight if error and empty
            }`}
            placeholder="e.g., QuizMasterFlex"
            required
            aria-describedby={
              errorMessage && !nickname.trim() ? "error-message" : undefined
            } // For accessibility
          />
        </div>
        ...
        <div className="mb-6">
          ...
          <input
            ...
            value={code}
            onChange={(e) =>
              handleInputChange(setCode, e.target.value.toUpperCase())
            } // Force uppercase on change & use wrapper
            maxLength={6}
            ...
            aria-describedby={
              errorMessage && !code.trim() ? "error-message" : undefined
            } // For accessibility
          />
        </div>
